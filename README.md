...
sequenceDiagram
    ...
    participant GW as ms-java-gateway<br>(Spring Cloud Gateway)
...
ms-py-agent 发起 GET /mcp/sse。
ms-java-biz 建立长连接，并立即推送 endpoint 事件，告知 ms-py-agent：“我在，发消息请 POST 到 /mcp/messages?sessionId=xyz”。

阶段二：握手 (Handshake)
ms-py-agent 发送 initialize 指令。
ms-java-biz 返回协议版本和能力声明（Capabilities）。
...
sequenceDiagram
    ...
    participant Py as ms-py-agent (大脑)<br>Writer
